...
{
    public partial class SpecialDBInsertDemo : Form
    {
        public SpecialDBInsertDemo()
        {
            InitializeComponent();
        }

        void InsertDBRecord(String QueryString, String[] ParameterNameArray, String[] ParameterValueArray, String[] IDValue, Boolean IsSealedDiffieHellman, Boolean IsXSalsa20Poly1305, ref String SampleJSONString, ref SpecialDBModel ReferenceModel, Boolean IsDebug = true)
        {
                        ...
                        if (ServerOnlineChecker == true)
                        {
                            ...
                            if (result.IsSuccessStatusCode)
                            {
                                ...
                                else
                                {
                                    ...
                                    MessageBox.Show("You have inserted a special db table record", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                }
                            }
                            else
                            {
            ...
            {
                MessageBox.Show("Please either input foreign key values or input foreign key values with parameter name and values", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void EncryptionTypeDeterminer(Object sender, EventArgs e)
        {
            ...
            {
                WebAPIURLTB.Text = "https://mrchewitsoftware.com.my:5002/SpecialSealedDHDBInsert/";
            }
            else
            {
                WebAPIURLTB.Text = "https://mrchewitsoftware.com.my:5002/SpecialSealedX3DHDBInsert/";
            }
        }

        private void InsertDBRecordBTN_Click(object sender, EventArgs e)
        {
            if (Directory.Exists(Application.StartupPath + "\\Application_Data\\SealedCredentials\\") == true && Directory.Exists(Application.StartupPath + "\\Application_Data\\DBCredentials\\") == true)
            {
                ...
                if (SQLQueryStringTB.Text != null && SQLQueryStringTB.Text.CompareTo("") != 0)
                {
                    if (SQLParameterNameArrayTB.Text != null && SQLParameterNameArrayTB.Text.CompareTo("") != 0 && SQLParameterValueArrayTB.Text != null && SQLParameterValueArrayTB.Text.CompareTo("") != 0) 
                    {
                        if (SQLKeyValueTB.Text != null && SQLKeyValueTB.Text.CompareTo("") != 0)
                        {
                            ...
                            InsertDBRecord(QueryString, ParameterNameArray, ParameterValueArray, IDValue, IsSealedDiffieHellman, IsXSalsa20Poly1305, ref SampleJSONString, ref MySpecialDBModel);
                    ...
                    else 
                    {
                        if (SQLKeyValueTB.Text != null && SQLKeyValueTB.Text.CompareTo("") != 0)
                        {
                            ...
                            IDValue = SQLKeyValue.Split(',');
                            IsSealedDiffieHellman = SealedDiffieHellmanRB.Checked;
                            //It doesn't matter if it's XSalsa20Poly1305 or XChaCha20Poly1305 anymore, as you want to insert only foreign key values instead of inserting foreign key and parameter values
                            InsertDBRecord(QueryString, ParameterNameArray, ParameterValueArray, IDValue, IsSealedDiffieHellman, true, ref SampleJSONString, ref MySpecialDBModel);
                            UniquePaymentIDTB.Text = MySpecialDBModel.UniquePaymentID;
                            SealedSessionIDTB.Text = MySpecialDBModel.MyDBCredentialModel.SealedSessionID;
                            SealedDBNameTB.Text = MySpecialDBModel.MyDBCredentialModel.SealedDBName;
                            SealedDBUserNameTB.Text = MySpecialDBModel.MyDBCredentialModel.SealedDBUserName;
                ...
                {
                    MessageBox.Show("SQL Query String must not be null or empty", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            else
            {
