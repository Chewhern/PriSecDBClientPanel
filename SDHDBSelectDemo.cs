                    ...
                    if (result.IsSuccessStatusCode)
                    {
                        ...
                        else
                        {
                            ...
                            while (LoopCount < MyRecordsModel.ParameterValues.Length) 
                            {
                                ...
                                try
                                {
                                    DecryptedValueByte = SodiumSealedPublicKeyBox.Open(SealedValueByte,RecipientECDHPKByte,RecipientECDHSKByte);
                                }
                                catch 
                                {
                                    IsSealedBox = false;
                                ...
                                if (IsSealedBox == false) 
                                {
                                    ...
                                    RecipientECDHSKByte = File.ReadAllBytes(Application.StartupPath + "\\Application_Data\\DHStorage\\SealedDHX25519SK.txt");
                                    SharedSecret = SodiumScalarMult.Mult(RecipientECDHSKByte, ServerPublicKeyByte);
                                    ...
                                    DecryptedValueByte = SodiumSecretBoxXChaCha20Poly1305.Open(SanitizedSealedValueByte, NonceByte, SharedSecret);
